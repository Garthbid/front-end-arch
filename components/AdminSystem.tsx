...
const BiddingWarsModule = ({ search, onSelect }: { search: string, onSelect: (id: string) => void }) => {
  ...
  return (
    <div className="p-0 animate-in fade-in duration-300">
      <AdminTable 
        headers={['Item Campaign', 'Bids', 'Status', 'User', 'Actions']}
        rows={filtered.map(item => (
          <tr key={item.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
            <td className="p-6">
              <div className="flex items-center gap-4">
            ...
            <td className="p-6">
               <div className="flex items-center gap-2">
                 ...
                 <span className="text-sm font-black text-slate-900">{item.bids}</span>
               </div>
            </td>
            <td className="p-6">
               <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${item.status === 'FEATURED' ? 'bg-[#ff5800] text-white' : 'bg-slate-100 text-slate-600'}`}>
                 {item.status || 'Active'}
               </span>
            </td>
            <td className="p-6 text-sm font-bold text-slate-500 italic">@{item.winningBidder}</td>
